...
export const GuideCard = ({ imageSrc, title, description, link }: { imageSrc: string, title: string, description: string, link: string }) => {
  return (
    <div className="relative flex flex-col items-center justify-center p-4 bg-white rounded shadow-lg transition-transform transform hover:scale-105 md:max-w-md w-full xl:max-w-xl mx-auto">
      <div className="relative w-full h-48 md:h-80">
        <Image src={imageSrc} alt={title} layout="fill" objectFit="cover" className="rounded-t" />
      </div>
      <div className="relative p-4 text-center">
        <h3 className="text-xl font-bold mb-2 text-[#264967] py-5">{title}</h3>
        <p className="text-[#264967] py-2">{description}</p>
